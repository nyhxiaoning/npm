...
function prettyView (packument, manifest) {
  ...
  return BB.try(() => {
    ...
    Object.keys(packument['dist-tags']).forEach((t) => {
      ...
      tags.push(`${style.bright(color.blue(t))}: ${version}`)
    })
    const info = {
      name: color.blue(manifest.name),
      version: color.blue(manifest.version),
      ...
      publisher: manifest._npmUser && unparsePerson({
        ...
        email: color.magenta(manifest._npmUser.email)
      }),
      modified: color.yellow(relativeDate(packument.time[packument.version])),
      collaborators: (packument.maintainers || []).map((u) => unparsePerson({
        name: color.yellow(u.name),
        email: color.magenta(u.email)
      })),
      repo: (
      ...
      tags,
      tarball: color.magenta(manifest.dist.tarball),
      ...
      unpackedSize: manifest.dist.unpackedSize && color.yellow(manifest.dist.unpackedSize)
    }
    ...
    if (info.bins.length) {
      console.log('')
      console.log('Binaries:', info.bins.join(', '))
    }

    if (info.repo || info.site) {
      console.log('')
      info.site && console.log('Website:', color.magenta(info.site))
      info.repo && console.log('Issues:', color.magenta(info.repo))
    }

    console.log('')
    console.log('Tarball URL:', info.tarball)
    console.log('Shasum:', info.shasum)
    info.integrity && console.log('Integrity:', info.integrity)
    info.fileCount && console.log('File Count:', info.fileCount)
    info.unpackedSize && console.log('Unpacked Size:', info.unpackedSize)
    ...
    if (info.collaborators && info.collaborators.length) {
      ...
      info.collaborators.forEach((u) => console.log('-', u))
    }
